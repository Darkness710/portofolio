...
<template>
    <header :class="[
        ...
        <nav class="mx-auto flex max-w-5xl items-center justify-between px-6 py-4">
            <a href="#home" class="font-display text-lg tracking-tight text-ivory" @click="closeMenu">
                Shiddiq Saiful Majid<span class="text-gold">.</span>
            </a>
            ...
            <button type="button" class="relative z-50 flex h-8 w-8 items-center justify-center text-ivory md:hidden"
                :aria-expanded="isOpen" aria-label="Buka menu navigasi" @click="isOpen = !isOpen">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <line x1="3" y1="12" x2="21" y2="12" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"
                        :class="['transition-opacity duration-300', isOpen && 'opacity-0']" />
        ...
        <div :class="[
            ...
            <ul class="flex h-full flex-col items-center justify-center gap-8">
                <li v-for="link in navLinks" :key="link.id">
                    <a :href="`#${link.id}`" class="font-display text-2xl text-ivory" @click="closeMenu">
                        {{ link.label }}
                    </a>
                </li>
            </ul>
